...
export const EQUAL_CHANCE_HIT = 55;
// kod/object/active/holder/nomoveon/battler/player.kod (constants block)
export const MAX_DAMAGE_PER_HIT = 30;
export const MAX_HEALTH_DAMAGE_FRACTION = 3;
...
export const STROKE_DAMAGE_FACTOR = 80;
export const MAX_PROFICIENCY_DAMAGE = 5;

// Unarmed: kod/object/passive/skill/stroke/unarmed.kod:35-36
export const UNARMED = { min: 1, max: 3, hit: 0 };

const T = Math.trunc;
export function bound(v, lo, hi) {
...
export function monsterOffense(level, difficulty) {
  return bound(3 * (level || 0) + 60 * (difficulty || 0), 1, 1500);
}
// monster.kod:4079 GetMaxHitPoints, then Fuzzy'd at :352
export function monsterHealthBase(level) {
  return level < 40 ? level : T((120 * level) / 100);
}
// monster.kod:4263 Fuzzy(n) = n - n/4 + random(0, n/2)  → 75%…125%
export function fuzzyLow(n) { return T(n) - T(T(n) / 4); }
export function fuzzyHigh(n) { return T(n) - T(T(n) / 4) + T((T(n) * 2) / 4); }

export function monsterHealth(level) {
...
export function playerOffense(ref, weapon) {
...
export function playerDefense(ref, weapon, worn) {
  const s = ref.skills, a = ref.stats;
...
export function hitChance(offense, defense) {
...
export function playerDamage(ref, weapon) {
...
export const ATCK_WEAP_ALL = 0x00001;
export function resistanceAgainst(worn, atype) {
...
export function damageToPlayer(raw, ref, worn, atype, aspell, { expected = true } = {}) {
  let dmg = raw;
  for (const w of worn) {
...
export function effective(ref, weapon, worn) {
...
export function matchup(ref, weapon, worn, creature, eff) {
