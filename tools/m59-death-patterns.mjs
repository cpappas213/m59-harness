...
// Windows argv[1] is a drive path, not a URL: `file://C:\...` parses with "c" as the HOST
// and the comparison never matches, so the tool silently refuses to run as a script. Use
// pathToFileURL, which is what every other standalone here does and what m59-path-test pins.
