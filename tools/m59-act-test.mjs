...
for (const file of files) {
  ...
  // ON WINDOWS AN ABSOLUTE PATH IS NOT A URL. `join` gives `C:\...`, and the ESM loader
  // reads the drive letter as a protocol: "Received protocol 'c:'". pathToFileURL is the
  // portable form, and m59-path-test.mjs exists to catch exactly this.
  const mod = await import(pathToFileURL(join(ACTDIR, file)).href);
