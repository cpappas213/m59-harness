...
export function launcherText(entry, {
  ...
  const wantHost = host ?? entry.host;
  const wantPort = port ?? entry.port;
  // Auto-detected unless the caller named an endpoint explicitly — an explicit --host/--port
  // is somebody saying where they want to go, and this must not overrule it.
  const via = (host == null && port == null) ? proxyFor(wantHost, wantPort) : null;
  const h = via ? '127.0.0.1' : wantHost;
  const p = via ? via.listen : wantPort;
  ...
  return [
    ...
    ...(via ? [`rem VIA THE PROXY on ${via.listen} (pid ${via.pid}) — it fronts ` +
               `${via.server.host}:${via.server.port}, so the broker can keep driving this`,
               'rem character while you watch it, and the packets pass through something',
               'rem that can record them. Re-generate this file to go direct.'] : []),
    ...
    'rem keep driving the character while you watch, and the packets pass through something',
    'rem that can record them. No proxy, or one for a different server, and this is direct.',
    `for /f "tokens=1,2" %%%%a in ('node "${join(REPO, 'tools', 'm59-devclient.mjs')}" ` +
      `--endpoint ${wantHost} ${wantPort}') do (set "M59H=%%%%a" & set "M59P=%%%%b")`,
    'if not defined M59H set "M59H=' + wantHost + '"',
    'if not defined M59P set "M59P=' + wantPort + '"',
    '',
    `cd /d "${dir}"`,
    `start "" /D "${dir}" "${client}" /H:%M59H% /P:%M59P% /U:${entry.account} /W:${entry.password} /Q`,
    '',
  ].join('\r\n');
}
