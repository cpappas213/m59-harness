...
for (const f of (fleet.fleet || [])) {
  ...
  rows.push({
    ...
    active_s: active,
    overhead_pct: active ? Math.round(100 * overhead / active) : 0,
    fighting_pct: active ? Math.round(100 * (t.fighting_s || 0) / active) : 0,
...
rows.sort((a, b) => (b.overhead_pct - b.fighting_pct) - (a.overhead_pct - a.fighting_pct));

if (JSON_OUT) { console.log(JSON.stringify(rows, null, 1)); }
else {
  const n = (v, w) => String(v ?? '?').padStart(w);
  console.log('worst first — overhead is travel + trade');
  console.log('ag   char        oh%  fight%  cast  e/h        purse  band   most-declined purchase');
  for (const r of rows)
    console.log(r.agent.padEnd(4), r.character.padEnd(10), n(r.overhead_pct, 4), n(r.fighting_pct, 6),
